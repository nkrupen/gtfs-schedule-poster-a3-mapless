...
class GTFSSchedulePoster:
    ...
    def _get_dynamic_layout_params(self, row_count, item_count):
        ...
        Font size strategy:
        Default to 3.8em.
        Reduce if density (rows + items/6) is high to prevent overflow.
        """
    ...
    def generate_poster(self, stop_id, date_label, city, school_week_start, holiday_week_start, output_file, skip_pdf_conversion=False):
        """
        Generates the poster HTML and optionally PDF.
        Returns the path to the PDF if generated, else the HTML.
        """
        try:
            ...
            bus_icon_raw = self._read_svg_candidates(
                ["/mnt/data/bus-icon.svg", "bus-icon.svg"]
            )
            ...
            logo_svg_inline = self._read_svg_candidates(["/mnt/data/logo.svg", "logo.svg"])
            alareuna_svg_inline = self._read_svg_candidates(["/mnt/data/alareuna.svg", "alareuna.svg"])
...
"""
    ...
    def generate_batch(self, input_string, date_label, city, school_week_start, holiday_week_start):
        ...
        for idx, stop_id in enumerate(stop_ids, 1):
            ...
            # Use stop_id as filename
            html_file = f"{stop_id}.html"
            
            # Generate the poster
            result_pdf = self.generate_poster(
                stop_id, 
                date_label, 
                city, 
                school_week_start, 
        ...
        # Zip the directory
        zip_filename = "schedule_posters" # shutil adds .zip automatically
        ...
        try:
            ...
            print(f"Not in Colab. File saved as {final_zip_name} in your working directory.")
...
if __name__ == "__main__":
    ...
    if os.path.exists(gtfs_file):
        ...
        city_input = input("Enter city for URLs (e.g., Kotka, Helsinki): ").strip()
        label_input = input("Enter date label (e.g., 10.8.2025–31.5.2026): ").strip()
        school_input = input("Enter School Week Monday (YYYY-MM-DD): ").strip()
        holiday_input = input("Enter Holiday Week Monday (YYYY-MM-DD): ").strip()
        
        # Fallbacks to ensure code doesn't break if user leaves fields empty
        city = city_input if city_input else "Kotka"
        date_label = label_input if label_input else "10.8.2025–31.5.2026"
        ...
        if user_input:
            gen.generate_batch(user_input, date_label, city, school_dt, holiday_dt)
        else:
            print("No stop numbers provided. Exiting.")
    else:
        print(f"❌ GTFS zip not found. Please ensure {gtfs_file} is in your working directory.")
