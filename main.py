...
class GTFSSchedulePoster:
    ...
    def generate_poster(self, stop_id, date_label, city, school_week_start, holiday_week_start, output_file, skip_pdf_conversion=False):
        try:
            ...
            bus_icon_raw = self._read_svg_candidates(["assets/bus-icon.svg"])
            ...
            schedule_url = f"https://{city_lower}.digitransit.fi/pysakit/{city_capital}:{stop_id}"
            ...
            if not logo_html.strip():
                print("⚠️ Warning: assets/logo.svg not found. Logo space will be empty.")

            alareuna_svg_inline = self._read_svg_candidates(["assets/alareuna.svg"])
            ...
            html = template.render(
                ...
                saturday_html=saturday_html,
                sunday_html=sunday_html,
                legend_html=legend_html,
                alareuna_svg_inline=alareuna_svg_inline,
    ...
    def generate_batch(self, input_string, date_label, city, school_start, holiday_start):
        ...
        for idx, stop_id in enumerate(stop_ids, 1):
            ...
            result_pdf = self.generate_poster(stop_id, date_label, city, school_start, holiday_start, html_file)
...
if __name__ == "__main__":
    ...
    if os.path.exists(gtfs_file):
        ...
        user_input = input("Enter stop numbers separated by comma (e.g., 155527,155528): ")
        
        if user_input.strip():
            city_input = input("Enter the city name for the QR code (e.g., Kotka, Kouvola, Helsinki): ").strip()
            date_label = input("Enter valid date label (e.g., 10.8.2025–31.5.2026): ").strip()
            school_start_str = input("Enter school week start Monday (YYYY-MM-DD): ").strip()
            holiday_start_str = input("Enter holiday week start Monday (YYYY-MM-DD): ").strip()

            try:
                school_start = datetime.strptime(school_start_str, "%Y-%m-%d")
                holiday_start = datetime.strptime(holiday_start_str, "%Y-%m-%d")
                
                gen.generate_batch(user_input, date_label, city_input, school_start, holiday_start)
            except ValueError:
                print("❌ Invalid date format. Please use YYYY-MM-DD.")
        else:
            print("No input provided.")
    else:
        print(f"❌ GTFS zip not found. Please place '{gtfs_file}' in the same directory as this script.")
